...
// 03_price_llm_field_only_v3 on a chosen network (bradbury OR localnet), with
// a per-run time budget, and emit structured JSON per run so downstream
// analysis can compute stats.
//
// Modeled 1:1 on batchRunV4.ts (same classifier, DV precedence, poll shape,
// summary format). Differences vs batchRunV4.ts:
//   * contract file: 03_price_llm_field_only_v3.py
//   * constructor args: (symbol, chain) — same as v02
//   * default budget: 420s (LLM leader path is slower than pure-deterministic
//                     v02/v4; give it more headroom before flagging TIMEOUT)
//
// Usage (from /backend):
//   node_modules/.bin/tsx scripts/batchRunV03.ts <network> <N> [budgetSeconds]
//     network        : "bradbury" | "localnet"
//     N              : integer in [1..50]
//     budgetSeconds  : optional, default 420 (per run)
//
// Example:
//   node_modules/.bin/tsx scripts/batchRunV03.ts bradbury 10 420
//   node_modules/.bin/tsx scripts/batchRunV03.ts localnet 25 180
...
const CONTRACT_FILE = "03_price_llm_field_only_v3.py";

// Constructor args for the price oracle: (symbol, chain).
...
// LLM leader path is meaningfully slower than pure-deterministic (v02/v4).
// Default budget widened to 420s so we don't spuriously TIMEOUT the healthy
// tail of the distribution.
const DEFAULT_BUDGET_SECONDS = 420;
...
// ---------------------------------------------------------------------------
// Phase 5d / 5e helpers (mirrors batchRunV4.ts / batchRunV02.ts)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Single run: deploy + resolve of one v03 contract.
// ---------------------------------------------------------------------------
...
function parseArgs(argv: readonly string[]): {
  ...
  if (!rawNet || !rawN) {
    throw new Error(
      "usage: tsx scripts/batchRunV03.ts <bradbury|localnet> <N> [budgetSeconds]",
    );
  }
...
async function main(): Promise<void> {
  ...
  // eslint-disable-next-line no-console
  console.log(
    `[batchRunV03] contract=${CONTRACT_FILE} network=${network} N=${n} budgetSeconds=${budgetSeconds} startedAt=${startedAtIso}`,
  );
